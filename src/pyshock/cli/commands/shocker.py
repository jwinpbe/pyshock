...
def info(*, shocker_id: Annotated[str, Parameter(alias="--id")]) -> None:
    """Get details for a shocker."""
    api = utils.get_api()
    config = get_config()
    acct_id = config.shocker_index.get(shocker_id)
    with console.status(
...
def shock(
    ...
    *,
    ...
    utils.send_operation(shocker_id, ShockerOperation.SHOCK, duration, intensity)
...
def vibrate(
    ...
    *,
    ...
    utils.send_operation(shocker_id, ShockerOperation.VIBRATE, duration, intensity)
...
def beep(
    *,
    ...
    utils.send_operation(shocker_id, ShockerOperation.BEEP, 500, 50)
