...
_code_group = Group.create_ordered("PiShock Share Codes")
...
code_app = App(name="code", help="Manage PiShock share codes.")
...
def _launcher(
    *tokens: Annotated[str, Parameter(show=False, allow_leading_hyphen=True)],
    ...
    try:
        command, bound, ignored = app.parse_args(tokens, print_error=False, exit_on_error=False)
    ...
    if "session" in ignored:
        session = utils.build_session(config, bound, account_id)
...
code_app.command("pyshock.cli.commands.code:add", help="Add a PiShock device by its share code.")
code_app.command("pyshock.cli.commands.code:delete", help="Remove a PiShock device by its share code.")
code_app.command("pyshock.cli.commands.code:list_codes", name="list", help="List PiShock shared devices.")
